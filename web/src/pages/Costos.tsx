...
import { obtenerAutoAprobadas, obtenerPendientesAprobacion } from '../api/pesadas'
...
export function Costos(props: PropsPagina) {
  ...
  return (
    <Marco
      {...props}
      titulo="Costos"
      descripcion="Vista de solo lectura. Aprobar o rechazar se sigue haciendo desde la estación de Centro de Costos."
      conectado={conectado}
    ...
    >
      {!datos ? (
        ...
        <>
          ...
          <TablaPendientes pesadas={datos.pendientes} />
          <TablaAutoAprobadas pesadas={datos.autoAprobadas} />
        </>
      )}
    </Marco>
  )
...
function TablaPendientes({ pesadas }: { pesadas: Pesada[] }) {
  return (
    <Seccion
          ...
          {pesadas.map((p) => {
            ...
            return (
              <tr key={p.id} className={FILA}>
                <td className={`${TD} font-semibold`}>{p.numero_ticket}</td>
...
function TablaAutoAprobadas({ pesadas }: { pesadas: Pesada[] }) {
  return (
    <Seccion
