# gui/maestros/empresas_transportistas_view.py — CRUD Empresas Transportistas
...
class EmpresasTransportistasView(ctk.CTkFrame):
    ...
    def _construir(self):
        ...
        ctk.CTkLabel(header, text="🚚 EMPRESAS TRANSPORTISTAS", font=ctk.CTkFont(family=UI["fuente"], size=15,weight="bold"), text_color=UI["color_accent"]).pack(side="left")
        ...
        self._entry_buscar = ctk.CTkEntry(left, placeholder_text="🔍 Buscar...", height=35, font=ctk.CTkFont(family=UI["fuente"], size=12))
        ...
        style = ttk.Style()
        style.configure("Transp.Treeview",
            background=UI["color_card"],
            foreground=UI["color_text"],
            fieldbackground=UI["color_card"],
            rowheight=26,
            font=("Segoe UI",11))
        style.configure("Transp.Treeview.Heading",
            background=UI["color_bg"],
            foreground=UI["color_text"],
            font=("Segoe UI",10,"bold"))
        style.map("Transp.Treeview",
            background=[("selected","#E0F2FE")],
            foreground=[("selected","#1E3A8A")])

        self._tree = ttk.Treeview(left, columns=("codigo","nombre","rif","telefono","estado"), show="headings", style="Transp.Treeview")
        ...
        ctk.CTkLabel(right, text="📝 Datos de la Empresa Transportista", font=ctk.CTkFont(family=UI["fuente"], size=14,weight="bold"), text_color=UI["color_text"]).pack(padx=18, pady=(15,10), anchor="w")
    ...
    def _filtrar(self):
        ...
        cargar_en_hilo(
            self, lambda: api_client.listar_maestro("empresas_transportistas", search=termino or None),
            on_exito=self._poblar_tabla,
            on_error=lambda e: messagebox.showerror("Error de conexión", str(e)),
        )
    ...
    def _on_select(self, event):
        ...
        pid = int(sel[0])
        try:
            p = api_client.obtener_maestro("empresas_transportistas", pid)
    ...
    def _guardar(self):
        ...
        if self._seleccionado_id:
            resultado = api_client.actualizar_maestro("empresas_transportistas", self._seleccionado_id, datos)
            mensaje_ok = "Empresa transportista actualizada"
        else:
            resultado = api_client.crear_maestro("empresas_transportistas", datos)
            mensaje_ok = "Empresa transportista creada"
        if resultado["exito"]:
    ...
    def _desactivar(self):
        ...
        if not messagebox.askyesno("Confirmar","¿Desactivar esta empresa transportista?"): return
        resultado = api_client.desactivar_maestro("empresas_transportistas", self._seleccionado_id, activo=False)
