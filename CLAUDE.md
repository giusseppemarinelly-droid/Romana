...
## Notas de sesión (2026-09-01)

Revisión completa de cinco ejes del proyecto (22 hallazgos: 5 críticos, 10 importantes, 7 menores). Se corrigieron cuatro críticos y un importante; el resto queda pendiente, con los de báscula atados a la prueba con hardware real del 2026-09-05.

- **`peso_entrada`: el 1er pesaje ahora es inmutable** (migración `e5f6a7b8c9d0`). `peso_bruto` cumplía dos funciones incompatibles — guardaba el peso de entrada y luego se sobrescribía con el mayor de los dos pesajes. Como la máquina de estados permite re-capturar tras un rechazo de CC, la segunda captura calculaba el neto contra el bruto de la captura anterior: entrada 15.000 con capturas de 40.000 → 40.100 daba un neto de **100 KG en vez de 25.100**, y el peso de entrada quedaba irrecuperable. `capturar_peso_salida()` calcula siempre contra `peso_entrada`; `peso_bruto`/`peso_tara` siguen siendo el mayor/menor de los dos pesajes (lo que muestran ticket, kardex y CC). **El backfill de la migración es deliberadamente parcial**: solo rellena las filas `en_planta`, donde el dato es cierto. En las ya capturadas, cuál de los dos pesos fue la entrada es genuinamente irrecuperable, así que quedan en NULL y el servicio se niega a re-capturarlas con un mensaje accionable en vez de adivinar.
- **Autenticación a nivel de router en `/pesadas`** (`backend/routers/pesadas.py`). Seis `GET` —listados, kardex, estadísticas— se habían escrito sin dependency y respondían 200 sin token; con `API_HOST=0.0.0.0` y CORS abierto, cualquier máquina de la red de planta podía bajar el histórico completo. Se declara `dependencies=[Depends(get_current_user)]` en el `APIRouter` y no ruta por ruta, para que el estado por defecto de una ruta nueva sea "protegida". Es autenticación, no autorización por rol: los cuatro niveles usan legítimamente esas lecturas.
- **El backend se niega a arrancar inseguro** (`backend.security.verificar_secreto_de_produccion`, llamada desde el `lifespan`). Si `JWT_SECRET_KEY` sigue siendo el default de desarrollo —que está en el repo, así que cualquiera puede firmarse un token de nivel 1— y el servidor va a escuchar en una interfaz de red, aborta con instrucciones. Se permite atado a loopback, que es como `main.py` levanta el backend de desarrollo: ese arranque automático ahora pasa `ROMANA_API_HOST=127.0.0.1` al subproceso.
- **El simulador dejó de ser respaldo automático** (`main.py`, `config.PERMITIR_SIMULADOR_COMO_RESPALDO`). Si la báscula real no conectaba, el sistema caía al simulador —pesos aleatorios de 15 a 55 toneladas— y nada en pantalla lo decía: se emitían tickets con kilos inventados. Ahora la estación arranca sin báscula (se ve) en vez de con pesos falsos (no se ve); para forzarlo hay `ROMANA_PERMITIR_SIMULADOR=1`. `hardware/display_manager.estado_display()` expone qué display está activo y `gui/components/header.py` muestra un badge permanente cuando hay algo que advertir.
- **Lectura del driver Toledo reescrita** (`hardware/display_toledo.py`). El display transmite en continuo, así que `reset_input_buffer()` + `readline()` dejaba el cursor a mitad de trama y devolvía un fragmento que no parseaba: en pantalla era "Sin señal" intermitente y en el botón de captura abortaba pesajes legítimos. Ahora se leen hasta `_MAX_LINEAS_POR_LECTURA` líneas quedándose con la primera que parsee, y se corta de inmediato si el puerto está mudo (cada `readline` vacío cuesta un timeout completo). Además: `OL`/`OV` (sobrecarga) se reconocen y se distinguen de "trama ilegible" vía `ultimo_error()`, y el peso conserva el signo — antes `abs()` convertía un peso negativo (celda descalibrada, basura en la plataforma) en un positivo plausible.
- **`DISPLAY` es overrideable por entorno** (`ROMANA_DISPLAY_MARCA` / `_PUERTO` / `_BAUDRATE`). Era lo único de `config.py` sin override, y el puerto cambia según a qué máquina esté conectada la báscula.
- **`diagnostico_bascula.py` ya no es interactivo** si recibe argumentos o si la entrada no es una consola: acepta `--puerto`, `--baudrate`, `--segundos`, `--barrer` (prueba los baudrates comunes) y `--listar`. Agrega volcado hexadecimal de cada trama, que es lo que resuelve de verdad el ancho del campo de peso: en texto, `'+      0'` y `'+000000'` se parecen demasiado. Corre bien desde WSL vía `powershell.exe … python diagnostico_bascula.py`.

### Pendientes de la revisión

C-04 (leer la báscula bloquea el hilo de Tkinter hasta 2 s por tick) y M-07 (el acceso al display no tiene lock) van juntos y quedan para después de la prueba con hardware. Siguen abiertos además: I-01 (carrera del contador de tickets), I-02 (`conectar()` da por buena la conexión con solo abrir el puerto), I-03 (ninguna captura exige peso estable ni valida contra `BASCULA`), I-04, I-05 (anular no registra autor), I-06, I-07 (el ticket cruza las horas en el flujo de descarga), I-09, I-10, y los menores M-01 a M-06.
